...
$$
\left(\sum_{i=1}^{N} \left( -\frac{\hbar^{2}}{2 m} \nabla_{i}^{2}-\frac{Z e^{2}}{4 \pi \varepsilon_{0} r_{i}}\right)+\sum_{i=1}^{N-1} \sum_{j=1+1}^{N} \frac{e^{2}}{4 \pi \varepsilon_{0} \left|{r}{i}-{r}{j}\right|}-E\right) \psi\left(\mathbf{r}{1} {r}{2} \ldots \mathbf{r}{N}\right)=0
$$
...
$$
\psi\left(\mathbf{r}{1}, \mathbf{r}{2}, \ldots, \mathbf{r}{N}\right)=\phi_{1}\left(\mathbf{r}{1}\right) \phi_{2}\left(\mathbf{r}{2}\right) \ldots \phi_{N}\left(\mathbf{r}{N}\right)
$$
...
- 负离子
  ...
  - He、N等不能形成负离子
  - 通常只有一个稳定的束缚能级，因而不能产生谱线（束缚-束缚跃迁），但是可以产生连续谱（束缚-自由跃迁），如太阳表面不透明度的重要来源 ${H-}$
...
- $\Delta L=0,\pm1$ 且不能从 0 到 0
  ...
  - $\Delta L$ 可以为0，只需 $\Delta l$ 不为 0（改变不同角动量之间夹角）
