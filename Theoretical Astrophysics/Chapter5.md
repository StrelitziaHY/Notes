...
- 恒星表面任一点 $(x,y,z)$ 的视向速度
  $$
  v_z=\vec{e} _ {z} \cdot (\vec\Omega\times \vec R)=\vec\Omega\cdot \left ( \vec R\times \vec e _ {z} \right ) =-x\Omega\sin{i}\equiv-\frac{x}{R}v_{eq}\sin i
  $$
...
对于球壳 $( r , r + \text{d}r )$ 
  ...
  - 单位体积内的扰动粒子数 $N$ 满足
    ...
    \Rightarrow W ^ { * } ( r ) =  e ^ { - \frac { 4 \pi } { 3 } N r ^ { 3 } },where W^*(0)=1 
    $$
...
- $n=6$ ，原子和中性扰动原子（极化场）相互作用，吸收原子和**其他**中性原子或者分子的碰撞——范德瓦尔斯力
  ...
  - $\overline { R _ { k } ^ { 2 } }$ 是高能级 $k$ 的轨道的平方平均半径，重要情形：
    ...
    - 对于 ${CaII}$ 的共振线H和K，$\overline { R _ { k } ^ { 2 } }=23a_0^2$ 
