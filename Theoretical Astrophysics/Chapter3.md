...
- **确定等价灰大气的单色辐射流**
   ...
   - 结论
     ...
     $$
     \frac { T } { T _ { { eff } } } = { \frac { 3 } { 4 } \left [  \tau + q ( \tau ) \right ] } ^ { 1 / 4 } = f ( \tau )
     $$
...
- 原理：$(\rho_1^{\ast},P_1^{\ast})$ 体元上升后**绝热膨胀**为 $(\rho_2^{\ast},P_2^{\ast})$ 体元，此时 $P_2^{\ast}=P_2$ ，即与外界压强相等；若 $\rho_2^{\ast}<\rho_2$ 则体元会继续上浮——不稳定；反之体元会开始下沉——稳定
